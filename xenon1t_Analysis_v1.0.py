from ROOT import *
from math import sqrt, pi, fabs
from array import array
from operator import add
import sys

#import psyco
#psyco.full()

#import pdb
#pdb.set_trace()

#offsets in the code with rock
Xoffset_rock = 0.
Yoffset_rock = 0.
Zoffset_rock = 0.

#set the detector dimensions
sensitive_volume_radius = 485.0 # mm
sensitive_volume_length = 962.0 # mm
cathode_pmt_distance = 14.5 # mm
Zoffset = 415.0

top_veto_low_z = 664.0 # mm
top_veto_radius = 550.0 # mm

bottom_veto_high_z = -751.0 # mm
bottom_veto_radius = 550.0 # mm

BC_high_z = -547.0 # mm
BC_low_z = -604.0 # mm
BC_radius = 485.0 # mm

#DATA PATH
root_files_directory = '/disk/xen04/data/xenon1t/cosmogenics'


root_file = sys.argv[1]

filename = root_files_directory + root_file
file = TFile(filename, 'read')
tree = file.Get('events/events')

nb_entries = tree.GetEntries()

newfilename = root_files_directory
newfile = TFile(root_files_directory + root_file[:-5] + '-t2_v1.0.root', 'recreate')
newtree = TTree('t2', '')

#che eto takoe??
max_size = 10000

eventid		= array('i', [0])
etot		= array('f', [0.])
ns			= array('i', [0])
ed 			= array('f', max_size*[0.])
xp 			= array('f', max_size*[0.])
yp 			= array('f', max_size*[0.])
zp 			= array('f', max_size*[0.])

etotv 		= array('f', [0.])
nsv 		= array('i', [0])
edv 		= array('f', max_size*[0.])
xpv 		= array('f', max_size*[0.])
ypv 		= array('f', max_size*[0.])
zpv 		= array('f', max_size*[0.])

etotBC 		= array('f', [0.])
nsBC 		= array('i', [0])
edBC 		= array('f', max_size*[0.])
xpBC 		= array('f', max_size*[0.])
ypBC 		= array('f', max_size*[0.])
zpBC 		= array('f', max_size*[0.])

xp_pri 		= array('f', [0.])
yp_pri 		= array('f', [0.])
zp_pri 		= array('f', [0.])

newtree.Branch('eventid', 	eventid,	'eventid/I')
newtree.Branch('etot',	 	etot, 		'etot/F')
newtree.Branch('ns', 		ns, 		'ns/I')
newtree.Branch('ed', 		ed, 		'ed[ns]/F')
newtree.Branch('xp', 		xp, 		'xp[ns]/F')
newtree.Branch('yp', 		yp, 		'yp[ns]/F')
newtree.Branch('zp', 		zp, 		'zp[ns]/F')

newtree.Branch('etotv', 	etotv, 		'etotv/F')
newtree.Branch('nsv', 		nsv, 		'nsv/I')
newtree.Branch('edv', 		edv, 		'edv[nsv]/F')
newtree.Branch('xpv', 		xpv, 		'xpv[nsv]/F')
newtree.Branch('ypv', 		ypv, 		'ypv[nsv]/F')
newtree.Branch('zpv', 		zpv, 		'zpv[nsv]/F')

newtree.Branch('etotBC', 	etotBC, 	'etotBC/F')
newtree.Branch('nsBC', 		nsBC, 		'nsBC/I')
newtree.Branch('edBC', 		edBC, 		'edBC[nsBC]/F')
newtree.Branch('xpBC', 		xpBC, 		'xpBC[nsBC]/F')
newtree.Branch('ypBC', 		ypBC, 		'ypBC[nsBC]/F')
newtree.Branch('zpBC', 		zpBC, 		'zpBC[nsBC]/F')

newtree.Branch('xp_pri', 	xp_pri, 	'xp_pri/F')
newtree.Branch('yp_pri', 	yp_pri, 	'yp_pri/F')
newtree.Branch('zp_pri', 	zp_pri, 	'zp_pri/F')

for entry in range(nb_entries):
#for entry in range(20000):
	tree.GetEntry(entry)

	if entry % 500 == 0:
		print '(%d/%d)' % (entry, nb_entries)
		#print '(%d/%d)' % (entry, 500)
	
	tree_xp, tree_yp, tree_zp = list(tree.xp), list(tree.yp), list(tree.zp)
	tree_ed = list(tree.ed)
	tree_type = list(tree.type)
	tree_creaproc = list(tree.creaproc)
	tree_edproc = list(tree.edproc)

	# build a list of energy depositions [[e0, [x0, y0, z0]], ...] for all steps, sorted in z
	numsteps = tree.nsteps
	all_steps = []
	for step in range(numsteps) :
		all_steps.append([tree_ed[step], [tree_xp[step], tree_yp[step], tree_zp[step]], tree_type[step], tree_creaproc[step], tree_edproc[step]])
	all_steps.sort(lambda p1, p2: int(p1[1][2]-p2[1][2]))

	# select only steps in the sensitive volume and with energy deposited
	filter_ed = lambda p: p[0] > 0.
	filter_zp = lambda p: p[1][2] < Zoffset+Zoffset_rock and p[1][2] > -sensitive_volume_length+Zoffset+Zoffset_rock
	filter_rp = lambda p: sqrt((p[1][0]-Xoffset_rock)**2 + (p[1][1]-Yoffset_rock)**2) < sensitive_volume_radius

	ed_steps = filter(filter_ed, all_steps)

	sv_steps = filter(filter_zp, ed_steps)
	sv_steps = filter(filter_rp, sv_steps)

	# compute energy deposition in the veto
	filter_side_veto = lambda p: sqrt((p[1][0]-Xoffset_rock)**2 + (p[1][1]-Yoffset_rock)**2) > sensitive_volume_radius
	filter_top_veto = lambda p: sqrt((p[1][0]-Xoffset_rock)**2 + (p[1][1]-Yoffset_rock)**2) < top_veto_radius and p[1][2] > top_veto_low_z
	filter_bottom_veto = lambda p: sqrt((p[1][0]-Xoffset_rock)**2 + (p[1][1]-Yoffset_rock)**2) < bottom_veto_radius and p[1][2] < bottom_veto_high_z+Zoffset_rock

	side_veto_steps = filter(filter_side_veto, ed_steps)
	top_veto_steps = filter(filter_top_veto, ed_steps)
	bottom_veto_steps = filter(filter_bottom_veto, ed_steps)

	veto_steps = side_veto_steps
	veto_steps += [step for step in top_veto_steps if step not in veto_steps]
	veto_steps += [step for step in bottom_veto_steps if step not in veto_steps]

	# compute energy deposition in the gamma-X region below the cathode
	#filter_BC = lambda p: sqrt((p[1][0]-Xoffset_rock)**2 + (p[1][1]-Yoffset_rock)**2) < BC_radius and p[1][2] < BC_high_z and p[1][2] > BC_low_z
	filter_BC_zp = lambda p: p[1][2] > BC_low_z and p[1][2] < BC_high_z
	filter_BC_rp = lambda p: sqrt((p[1][0]-Xoffset_rock)**2 + (p[1][1]-Yoffset_rock)**2) < BC_radius

	BC_steps = filter(filter_BC_zp, ed_steps)
	BC_steps = filter(filter_BC_rp, BC_steps)


	############################################################
	# compute the number of scatters in the sensitive volume
	if len(sv_steps) > 0:
		# start with 1 scatter
		nb_s = 1

		s_etot = sv_steps[0][0]
			
		s_ed = sv_steps[0][0]
		s_xp = sv_steps[0][1][0]*sv_steps[0][0]
		s_yp = sv_steps[0][1][1]*sv_steps[0][0]
		s_zp = sv_steps[0][1][2]*sv_steps[0][0]

		# for all steps starting with the second one
		for step in range(1, len(sv_steps)):
			step_ed = sv_steps[step][0]
			step_xp, step_yp, step_zp = sv_steps[step][1][0], sv_steps[step][1][1], sv_steps[step][1][2]

			# if the step if separated enough
			if fabs(step_zp - sv_steps[step-1][1][2]) > 3.0:
				# then this is a new scatter

				# save the energy deposited and the weighted position of the previous scatter
				ed[nb_s-1] = s_ed
				xp[nb_s-1] = s_xp/s_ed - Xoffset_rock
				yp[nb_s-1] = s_yp/s_ed - Yoffset_rock
				zp[nb_s-1] = s_zp/s_ed - Zoffset_rock

				s_etot += step_ed

				# start a new scatter
				nb_s += 1
				s_ed = step_ed
				s_xp, s_yp, s_zp = step_xp*step_ed, step_yp*step_ed, step_zp*step_ed

			else:
				# this is not a new scatter, keep adding in the current
				s_etot += step_ed
				s_ed += step_ed
				s_xp += step_xp*step_ed
				s_yp += step_yp*step_ed
				s_zp += step_zp*step_ed

		# save the last scatter (or the first if we have only one!)
		ed[nb_s-1] = s_ed
		xp[nb_s-1] = s_xp/s_ed - Xoffset_rock
		yp[nb_s-1] = s_yp/s_ed - Yoffset_rock
		zp[nb_s-1] = s_zp/s_ed - Zoffset_rock
	else:
		nb_s = 0


	################################################
	# compute the number of scatters in the veto
	if len(veto_steps) > 0:
		# start with 1 scatter
		nb_vs = 1

		vs_etot = veto_steps[0][0]
			
		vs_ed = veto_steps[0][0]
		vs_xp = veto_steps[0][1][0]*veto_steps[0][0]
		vs_yp = veto_steps[0][1][1]*veto_steps[0][0]
		vs_zp = veto_steps[0][1][2]*veto_steps[0][0]

		# for all steps starting with the second one
		for step in range(1, len(veto_steps)):
			step_ed = veto_steps[step][0]
			step_xp, step_yp, step_zp = veto_steps[step][1][0], veto_steps[step][1][1], veto_steps[step][1][2]

			# if the step if separated enough
			if fabs(sqrt((step_xp - veto_steps[step-1][1][0])**2 + (step_yp - veto_steps[step-1][1][1])**2 + (step_zp - veto_steps[step-1][1][2])**2)) > 10.0:
				# then this is a new scatter

				# save the energy deposited and the weighted position of the previous scatter
				edv[nb_vs-1] = vs_ed
				xpv[nb_vs-1] = vs_xp/vs_ed - Xoffset_rock
				ypv[nb_vs-1] = vs_yp/vs_ed - Yoffset_rock
				zpv[nb_vs-1] = vs_zp/vs_ed - Zoffset_rock

				vs_etot += step_ed

				# start a new scatter
				nb_vs += 1
				vs_ed = step_ed
				vs_xp, vs_yp, vs_zp = step_xp*step_ed, step_yp*step_ed, step_zp*step_ed

			else:
				# this is not a new scatter, keep adding in the current
				vs_etot += step_ed
				vs_ed += step_ed
				vs_xp += step_xp*step_ed
				vs_yp += step_yp*step_ed
				vs_zp += step_zp*step_ed

		# save the last scatter (or the first if we have only one!)
		edv[nb_vs-1] = vs_ed
		xpv[nb_vs-1] = vs_xp/vs_ed - Xoffset_rock
		ypv[nb_vs-1] = vs_yp/vs_ed - Yoffset_rock
		zpv[nb_vs-1] = vs_zp/vs_ed - Zoffset_rock
	else:
		nb_vs = 0
		vs_etot = 0

	################################################
	# compute the number of scatters below the cathode
	if len(BC_steps) > 0:
		# start with 1 scatter
		nb_BC = 1

		BC_etot = BC_steps[0][0]
			
		BC_ed = BC_steps[0][0]
		BC_xp = BC_steps[0][1][0]*BC_steps[0][0]
		BC_yp = BC_steps[0][1][1]*BC_steps[0][0]
		BC_zp = BC_steps[0][1][2]*BC_steps[0][0]

		# for all steps starting with the second one
		for step in range(1, len(BC_steps)):
			BC_ed = BC_steps[step][0]
			step_xp, step_yp, step_zp = BC_steps[step][1][0], BC_steps[step][1][1], BC_steps[step][1][2]

			# if the step if separated enough
			#if fabs(sqrt((stepnr_xp - BCnr_steps[step-1][1][0])**2 + (stepnr_yp - BCnr_steps[step-1][1][1])**2 + (stepnr_zp - BCnr_steps[step-1][1][2])**2)) > 3.0:
			if fabs(step_zp - BC_steps[step-1][1][2]) > 3.0:
				# then this is a new scatter

				# save the energy deposited and the weighted position of the previous scatter
				edBC[nb_vs-1] = BC_ed
				xpBC[nb_vs-1] = BC_xp/BC_ed - Xoffset_rock
				ypBC[nb_vs-1] = BC_yp/BC_ed - Yoffset_rock
				zpBC[nb_vs-1] = BC_zp/BC_ed - Zoffset_rock

				BC_etot += step_ed

				# start a new scatter
				nb_BC += 1
				BC_ed = step_ed
				BC_xp, BC_yp, BC_zp = step_xp*step_ed, step_yp*step_ed, step_zp*step_ed

			else:
				# this is not a new scatter, keep adding in the current
				BC_etot += step_ed
				BC_ed += step_ed
				BC_xp += step_xp*step_ed
				BC_yp += step_yp*step_ed
				BC_zp += step_zp*step_ed

		# save the last scatter (or the first if we have only one!)
		edBC[nb_BC-1] = BC_ed
		xpBC[nb_BC-1] = BC_xp/BC_ed - Xoffset_rock
		ypBC[nb_BC-1] = BC_yp/BC_ed - Yoffset_rock
		zpBC[nb_BC-1] = BC_zp/BC_ed - Zoffset_rock
	else:
		nb_BC = 0
		BC_etot = 0



	########################
	# save event information
	if nb_s>0:
		# save event information
		eventid[0] = tree.eventid
		etot[0] = s_etot
		ns[0] = nb_s
		# veto
		etotv[0] = vs_etot 
		nsv[0] = nb_vs
		# BC
		etotBC[0] = BC_etot 
		nsBC[0] = nb_BC
		# primaries
		xp_pri[0] = tree.xp_pri
		yp_pri[0] = tree.yp_pri
		zp_pri[0] = tree.zp_pri

		# fill the tree
		newtree.Fill()

file.Close()

newfile.Write()
newfile.Close()

