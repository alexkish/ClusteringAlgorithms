...
newtree.Branch('e_pri', 	zp_pri, 	'e_pri/F')
	...
	if len(svnr_steps) > 0:
		...
		for step in range(1, len(svnr_steps)):
			...
			if fabs(stepnr_zp - svnr_steps[step-1][1][2]) > 3.0:
				...
				if fabs(sqrt((stepnr_xp - svnr_steps[step-1][1][0])**2 + (stepnr_yp - svnr_steps[step-1][1][1])**2)) >10.0:
	...
	if len(sver_steps) > 0:
		...
		for step in range(1, len(sver_steps)):
			...
			if fabs(steper_zp - sver_steps[step-1][1][2]) > 3.0:
				...
				zp_ER[nber_s-1] = ser_zp/ser_ed - Zoffset
				...
				if fabs(sqrt((steper_xp - sver_steps[step-1][1][0])**2 + (steper_yp - sver_steps[step-1][1][1])**2)) >10.0:
