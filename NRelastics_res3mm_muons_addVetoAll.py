...
#root_files_directory = '/home/physik/alexkish/data/Xe100_MuonsDisk_1e5/'
#xecluster
#root_files_directory='/archive/mc/xenon100/alexkish/CalibrationSources/'
#root_files_directory='/archive/mc/xenon100/alexkish/bg/neutrons/'
#root_files_directory='/archive/mc/xenon100/alexkish/bg/muons/'
# idark05x
root_files_directory='/Users/alexkish/data/MC/muons/'
...
newfile = TFile(root_files_directory + root_file[:-5] + '-NRelastics-res3mm-addVetoAll.root', 'recreate')
newtree = TTree('t2', '')
...
#for entry in range(1000):
	...
	# EM component
	#filter_er = lambda p: p[2] != 'gamma' and p[2] != 'e-' and p[2] != 'e+'
	#Elastic nuclear recoils
	filter_nr = lambda p: p[3] == 'hElastic'
	filter_er = lambda p: p[0] > 0.
	...
	sv_steps = filter(filter_zp, nznr_steps)
	...
	side_veto_steps = filter(filter_side_veto, nzer_steps)
	top_veto_steps = filter(filter_top_veto, nzer_steps)
	bottom_veto_steps = filter(filter_bottom_veto, nzer_steps)
